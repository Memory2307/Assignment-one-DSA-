...
public isolated client class CarRentalServiceEmptyCaller {
    private final grpc:Caller caller;
...
public isolated client class CarRentalServiceListCarsResponseCaller {
    private final grpc:Caller caller;
    ...
    isolated remote function sendListCarsResponse(ListCarsResponse response) returns grpc:Error? {
        return self.caller->send(response);
    }

    isolated remote function sendContextListCarsResponse(ContextListCarsResponse response) returns grpc:Error? {
        return self.caller->send(response);
    }
